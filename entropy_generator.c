    ...
    if (get_failure() == 0) {
        ...
        while (fgets(buffer, sizeof(buffer), fp) != NULL) {
            if (strncmp(buffer, "cpu", 3) == 0) {
                uint64_t values[4];
                if (sscanf(buffer, "cpu %lu %lu %lu %lu", &values[0], &values[1], &values[2], &values[3]) == 4) {
...
// Modified seed_rng to return the generated seed as a dynamically allocated array
unsigned char* seed_rng(size_t* seed_size) {
    unsigned long long int entropy = collect_entropy();  // Collect entropy

    if (*seed_size < SHA256_DIGEST_LENGTH) {
        fprintf(stderr, "Error: seed_size must be at least %d bytes\n", SHA256_DIGEST_LENGTH);
        return NULL;
    }

    // Dynamically allocate memory for the seed
    unsigned char *seed = (unsigned char*)malloc(SHA256_DIGEST_LENGTH);
    if (seed == NULL) {
        perror("Error allocating memory for seed");
        return NULL;  // Return NULL if memory allocation fails
    }

    // Use SHA-256 to create a 32-byte seed from the entropy
    SHA256((unsigned char*)&entropy, sizeof(entropy), seed);

    return seed;  // Return the pointer to the generated seed
}
//I have no idea if this process makes any sense, does that make it better or worse?
